...
def analyze_condition(df_weather, is_weather_live, river_info, user_logs, target_river, target_date, current_actual):
    ...
    t_slight = river_info.get("thresholds", {"slight": 0.15})["slight"]
    t_hard = river_info.get("thresholds", {"hard": 0.30})["hard"]
    t_danger = river_info.get("thresholds", {"danger": 0.50})["danger"]
    
    if level_diff < -0.10: level_trend = f"渇水 ({level_diff*100:+.0f}cm)"
    elif level_diff < t_slight: level_trend = f"平水 ({level_diff*100:+.0f}cm)"
    elif level_diff < t_hard: level_trend = f"やや高水 ({level_diff*100:+.0f}cm)"
